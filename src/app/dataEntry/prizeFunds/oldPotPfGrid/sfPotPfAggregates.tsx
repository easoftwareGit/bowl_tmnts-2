...
/**
 * Create the total aggregates for the pot prize fund grid
 *
 * @returns {syncFusionAggregateDef[]} - array of aggregates
 */
export const createPotPfTotalAggregates = (): syncFusionAggregateDef[] => {
...
/**
 * Create the difference (totalPrizeFund - sum of position amounts) aggregates for the pot prize fund grid
 *
 * @param {number} totalPrizeFund - the total amount of the prize fund
 * @returns {syncFusionAggregateDef[]} - array of aggregates
 */
export const createPotPfDiffAggregates = (
  ...
  const totalAmount = (data: any): number => {
    ...
    // sum the amounts
    return rows.reduce(
      (sum: number, row: potPfEntryRow) => sum + (row.amount ?? 0),
      0,
    );
  };
