...
export default function ElimPrizeFundEntry() {
  const params = useParams();
  const tmntId = params.tmntId as string;
  const elimId = params.elimId as string;  
  ...
  // get original elimId for change detection
  const initializedElimIdRef = useRef<string | null>(null);
  
  const [cashers, setCashers] = useState(1);
  const [cashersErr, setCashersErr] = useState("");
  const [expenses, setExpenses] = useState(elimExpenses);  
  const [expensesText, setExpensesText] = useState(elimExpenses.toFixed(2));
  const [totalElimPrizeFund, setTotalElimPrizeFund] = useState(elimEntryFees - elimExpenses);
   ...
   *
   * ElimPrizeFundGrid forwards this ref to PrizeFundGrid. PrizeFundGrid then
  ...
  useEffect(() => {
    ...
    const numCashers = elimPfs.length;
    const prizeFunds = elimPfsToPrizeFunds(elimPfs);
    ...
    setExpenses(elimExpenses);
    setExpensesText(elimExpenses.toFixed(2));
    setPrizeFund(elimPrizeFund);    

    lastCashersRef.current = numCashers;
    lastExpensesRef.current = elimExpenses;
  ...
  * The maximum expenses is the elimPrizeFund   
  * 
  ...
  */ 
  const getSafeExpenses = (value: number): number => { 
    const maxExpenses = elimPrizeFund <= 0 ? 0 : elimPrizeFund; 
  ...
  */ 
  const applyEnteredExpenses = (value: number | null): void => {        
    if (value == null || Number.isNaN(value)) {
      ...
      setTotalElimPrizeFund(elimEntryFees);
      return;
    }
    ...
    const allGamesPf = elimEntryFees - safeValue;
    setTotalElimPrizeFund(allGamesPf);
          ...
          <div className="prizeFundForm">
            <div className="justify-content-left">
              ...
              <h5>
                ...
                <div className="row">
                  <div className="col-5">Eliminator:</div>
                  <div className="col-7">{elimName}</div>
                </div>
              </h5>
            </div>
