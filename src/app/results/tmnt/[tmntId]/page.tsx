...
import { fetchOneTmntGameResults, getOneTmntGameResultsError, getOneTmntGameResultsLoadStatus, selectOneTmntGameResults } from "@/redux/features/oneTmntGameResults/oneTmntGameResultsSlice";
...
export const DivResultsPage = () => { 
  ...
  useEffect(() => {
    if (tmntResults && tmntResults.length > 0) {
      ...
      setTmntStartDate(getMonthDay(startDate) + ', ' + startDate.substring(0, 4));
      const tmntDivIds = Array.from(new Set(tmntResults.map(result => result.div_id)));      
  ...
  return (
    <>
      ...
      {resultsLoadStatsus !== 'loading' && resultsLoadStatsus !== 'succeeded' && resultsError
        ? (<div>Error: {resultsError} tmntLoadStatus: {resultsLoadStatsus}</div>
        ) : null}
