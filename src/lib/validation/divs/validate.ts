...
export const validHdcpPer = (hdcpPer: unknown): boolean => {
  if (typeof hdcpPer !== 'number' || !Number.isFinite(hdcpPer)) return false  
  return (hdcpPer >= minHdcpPer && hdcpPer <= maxHdcpPer);
}
export const validHdcpFrom = (hdcpFrom: unknown): boolean => {
...
export const validHdcpFor = (hdcpFor: unknown): boolean => {
  if (hdcpFor == null || typeof hdcpFor !== 'string') return false
 ...
 * @returns {ErrorCode.NONE | ErrorCode.INVALID_DATA, ErrorCode.OtherError}
 */
const validDivData = (div: divType): ErrorCode => {
  try {
    if (!div) return ErrorCode.INVALID_DATA;
 ...
 * 
 * @param div - div data to validate
 * @returns {ErrorCode.NONE | ErrorCode.MISSING_DATA | ErrorCode.INVALID_DATA | ErrorCode.OTHER_ERROR}
 */
 ...
 * @returns {validDivsType} - {divs:divType[], errorCode: ErrorCode.NONE | ErrorCode.MISSING_DATA | ErrorCode.INVALID_DATA | ErrorCode.OtherError}
 */
export const validateDivs = (divs: divType[]): validDivsType => {
  ...
  if (!Array.isArray(divs) || divs.length === 0) {
    return { divs: blankDivs, errorCode: ErrorCode.MISSING_DATA };
  };
  ...
  while (i < divs.length) {
    ...
    if (i > 0 && tmntId !== toPost.tmnt_id) {
      return { divs: okDivs, errorCode: ErrorCode.INVALID_DATA };
    }
