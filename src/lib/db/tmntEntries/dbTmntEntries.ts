...
import { allEntriesOneSquadType, brktEntryType, dataOneSquadEntriesType, divEntryType,  elimEntryType, playerType, potEntryType, putManyEntriesReturnType, tmntEntryBrktEntryType, tmntEntryDivEntryType, tmntEntryElimEntryType, tmntEntryPlayerType, tmntEntryPotEntryType, updatedEntriesCountsType } from "@/lib/types/types";
 ...
 */
const getPlayersToSave = (editedPlayers: playerType[], origPlayers: playerType[]) => { 

  const toSave: tmntEntryPlayerType[] = [];
  if (!editedPlayers || editedPlayers.length === 0 || !origPlayers || origPlayers.length === 0) return toSave;
  // get deleted players
  origPlayers.forEach((origPlayer: playerType) => { 
    if (isValidBtDbId(origPlayer.id, "ply")) {
      const foundPlayer = editedPlayers.find((player) => player.id === origPlayer.id);
      if (!foundPlayer) {
  ...
  editedPlayers.forEach((editedPlayer: playerType) => { 
    if (isValidBtDbId(editedPlayer.id, "ply")) {
      const foundPlayer = origPlayers.find((player) => player.id === editedPlayer.id);      
      if (foundPlayer) {
 ...
 */
const getDivEntriesToSave = (editedDivEntries: divEntryType[], origDivEntries: divEntryType[]) => { 
  const toSave: tmntEntryDivEntryType[] = [];
  if (!editedDivEntries || editedDivEntries.length === 0 || !origDivEntries) return toSave;
  
  origDivEntries.forEach((origDivEntry: divEntryType) => { 
    if (isValidBtDbId(origDivEntry.id, "den")) {
      const foundDivEntry = editedDivEntries.find((divEntry) => divEntry.div_id === origDivEntry.div_id && divEntry.player_id === origDivEntry.player_id);
      if (!foundDivEntry) {
  ...
  editedDivEntries.forEach((editedDivEntry: divEntryType) => { 
    const foundDivEntry = origDivEntries.find((divEntry) => divEntry.div_id === editedDivEntry.div_id && divEntry.player_id === editedDivEntry.player_id);
    if (foundDivEntry) {
      ...
      if (JSON.stringify({ ...foundDivEntry, id: "" }) !== JSON.stringify({ ...editedDivEntry, id: "" })) {
 ...
 */
const getPotEntriesToSave = (editedPotEntries: potEntryType[], origPotEntries: potEntryType[], squadId: string) => { 
  const toSave: tmntEntryPotEntryType[] = [];
  if (!editedPotEntries || editedPotEntries.length === 0 || !origPotEntries) return toSave;
  
  origPotEntries.forEach((origPotEntry: potEntryType) => { 
    if (isValidBtDbId(origPotEntry.id, "pen")) {
      const foundPotEntry = editedPotEntries.find((potEntry) => potEntry.pot_id === origPotEntry.pot_id && potEntry.player_id === origPotEntry.player_id);
      if (!foundPotEntry) {
  ...
  editedPotEntries.forEach((editedPotEntry: potEntryType) => { 
    if (isValidBtDbId(editedPotEntry.id, "pen")) {
      const foundPotEntry = origPotEntries.find((potEntry) => potEntry.pot_id === editedPotEntry.pot_id && potEntry.player_id === editedPotEntry.player_id);      
      if (foundPotEntry) {
        if (JSON.stringify({ ...foundPotEntry, id: "" }) !== JSON.stringify({ ...editedPotEntry, id: "" })) {        
 ...
 */
const getBrktEntriesToSave = (editedBrktEntries: brktEntryType[], origBrktEntries: brktEntryType[]) => { 
  const toSave: tmntEntryBrktEntryType[] = [];
  if (!editedBrktEntries || editedBrktEntries.length === 0 || !origBrktEntries) return toSave;
    
  origBrktEntries.forEach((origBrktEntry: brktEntryType) => { 
    if (isValidBtDbId(origBrktEntry.id, "ben")) {
      const foundBrktEntry = editedBrktEntries.find((brktEntry) => brktEntry.brkt_id === origBrktEntry.brkt_id && brktEntry.player_id === origBrktEntry.player_id);
      if (!foundBrktEntry) {
  ...
  editedBrktEntries.forEach((editedBrktEntry: brktEntryType) => { 
    if (isValidBtDbId(editedBrktEntry.id, "ben")) {
      const foundBrktEntry = origBrktEntries.find((brktEntry) => brktEntry.brkt_id === editedBrktEntry.brkt_id && brktEntry.player_id === editedBrktEntry.player_id);      
      if (foundBrktEntry) {
        if (JSON.stringify({ ...foundBrktEntry, id: "" }) !== JSON.stringify({ ...editedBrktEntry, id: "" })) {        
 ...
 */
const getElimEntriesToSave = (editedElimEntries: elimEntryType[], origElimEntries: elimEntryType[]) => {

  const toSave: tmntEntryElimEntryType[] = [];
  if (!editedElimEntries || editedElimEntries.length === 0 || !origElimEntries) return toSave;
    
  origElimEntries.forEach((origElimEntry: elimEntryType) => { 
    if (isValidBtDbId(origElimEntry.id, "een")) {
      const foundElimEntry = editedElimEntries.find((elimEntry) => elimEntry.elim_id === origElimEntry.elim_id && elimEntry.player_id === origElimEntry.player_id);
      if (!foundElimEntry) {
  ...
  editedElimEntries.forEach((editedElimEntry: elimEntryType) => { 
    if (isValidBtDbId(editedElimEntry.id, "een")) {
      const foundElimEntry = origElimEntries.find((elimEntry) => elimEntry.elim_id === editedElimEntry.elim_id && elimEntry.player_id === editedElimEntry.player_id);      
      if (foundElimEntry) {
        if (JSON.stringify({...foundElimEntry, id: ""}) !== JSON.stringify({ ...editedElimEntry, id: "" })) { 
...
export const gotUpdateErrors = (updateCounts: putManyEntriesReturnType) => {
  ...
  if (updateCounts.players.deletes < 0 || updateCounts.players.updates < 0 || updateCounts.players.inserts < 0 ||
    updateCounts.divEntries.deletes < 0 || updateCounts.divEntries.updates < 0 || updateCounts.divEntries.inserts < 0 ||
    updateCounts.potEntries.deletes < 0 || updateCounts.potEntries.updates < 0 || updateCounts.potEntries.inserts < 0 ||
    updateCounts.brktEntries.deletes < 0 || updateCounts.brktEntries.updates < 0 || updateCounts.brktEntries.inserts < 0 ||
    updateCounts.elimEntries.deletes < 0 || updateCounts.elimEntries.updates < 0 || updateCounts.elimEntries.inserts < 0) {
      return true      
  }
  return false;
}
...
export const getTotalUpdated = (updateInfo: putManyEntriesReturnType) => {
  ...
  return updateInfo.players.deletes + updateInfo.players.updates + updateInfo.players.inserts +
    updateInfo.divEntries.deletes + updateInfo.divEntries.updates + updateInfo.divEntries.inserts +
    updateInfo.potEntries.deletes + updateInfo.potEntries.updates + updateInfo.potEntries.inserts +
    updateInfo.brktEntries.deletes + updateInfo.brktEntries.updates + updateInfo.brktEntries.inserts +
    updateInfo.elimEntries.deletes + updateInfo.elimEntries.updates + updateInfo.elimEntries.inserts;
}
 ...
 */
const updateCurrentPlayers = (playersToSave: tmntEntryPlayerType[], allEntries: allEntriesOneSquadType): playerType[] | null => {
  ...
  toDelete.forEach((player: tmntEntryPlayerType) => {
    const curIndex = updatedPlayers.findIndex((curPlayer) => curPlayer.id === player.id);
    if (curIndex !== -1) {      
 ...
 */
const updateCurrentDivEntries = (divEntriesToSave: tmntEntryDivEntryType[],  allEntries: allEntriesOneSquadType): divEntryType[] | null => {
  ...
  // deletes
  const toDelete = divEntriesToSave.filter((divEntry) => divEntry.eType === 'd');
  toDelete.forEach((divEntry: tmntEntryDivEntryType) => {
    const curIndex = updatedDivEntries.findIndex((curDivEntry) => divEntry.div_id === curDivEntry.div_id && divEntry.player_id === curDivEntry.player_id);        
    if (curIndex !== -1) {
  ...
  // updates
  const toUpdate = divEntriesToSave.filter((divEntry) => divEntry.eType === 'u');
  ...
  // inserts
  const toInsert = divEntriesToSave.filter((divEntry) => divEntry.eType === 'i');
 ...
 */
const updateCurrentPotEntries = (potEntriesToSave: tmntEntryPotEntryType[],  allEntries: allEntriesOneSquadType): potEntryType[] | null => {
  ...
  // deletes
  const toDelete = potEntriesToSave.filter((potEntry) => potEntry.eType === 'd');
  toDelete.forEach((potEntry: tmntEntryPotEntryType) => {
    const curIndex = updatedPotEntries.findIndex((curPotEntry) => potEntry.pot_id === curPotEntry.pot_id && potEntry.player_id === curPotEntry.player_id);
    if (curIndex !== -1) {
  ...
  // updates
  const toUpdate = potEntriesToSave.filter((potEntry) => potEntry.eType === 'u');
  ...
  // inserts
  const toInsert = potEntriesToSave.filter((potEntry) => potEntry.eType === 'i');
 ...
 */
const updateCurrentBrktEntries = (brktEntriesToSave: tmntEntryBrktEntryType[],  allEntries: allEntriesOneSquadType): brktEntryType[] | null => {
  ...
  // deletes
  const toDelete = brktEntriesToSave.filter((brktEntry) => brktEntry.eType === 'd');
  toDelete.forEach((brktEntry: tmntEntryBrktEntryType) => {
    const curIndex = updatedBrktEntries.findIndex((curBrktEntry) => brktEntry.brkt_id === curBrktEntry.brkt_id && brktEntry.player_id === curBrktEntry.player_id);
    if (curIndex !== -1) {
  ...
  // updates
  const toUpdate = brktEntriesToSave.filter((brktEntry) => brktEntry.eType === 'u');
  ...
  // inserts
  const toInsert = brktEntriesToSave.filter((brktEntry) => brktEntry.eType === 'i');
...
/**
 * updates allEntriesOneSquadType.curData tables with new data
 * 
 * @param updateInfo - object with data to update
 * @param allEntries - object with counts of updates
 * @returns {updatedEntriesCountsType} - object with counts of updates
 */
// export const updateOneSquadEntries = (updateInfo: putManyEntriesReturnType, allEntries: allEntriesOneSquadType): updatedEntriesCountsType => {

//   const updateCount: updatedEntriesCountsType = {
//     players: 0,
//     divEntries: 0,
//     potEntries: 0,
//     brktEntries: 0,
...
//     total: 0
//   }    
//   updateCount.players = updateCurrentPlayers(updateInfo.playersToUpdate, allEntries);
//   updateCount.divEntries = updateCurrentDivEntries(updateInfo.divEntriesToUpdate, allEntries);
//   updateCount.potEntries = updateCurrentPotEntries(updateInfo.potEntriesToUpdate, allEntries);
//   updateCount.brktEntries = updateCurrentBrktEntries(updateInfo.brktEntriesToUpdate, allEntries);
//   updateCount.elimEntries = updateCurrentElimEntries(updateInfo.elimEntriesToUpdate, allEntries);
//   updateCount.total = updateCount.players + updateCount.divEntries + updateCount.potEntries + updateCount.brktEntries + updateCount.elimEntries
//   return updateCount
// }
