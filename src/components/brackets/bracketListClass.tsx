...
export class BracketList { 
  ...
  private _dupThreshold = 12; // can have a duplicate match 1 in 10 times
  ...
  calcTotalBrkts(playerEntries: (typeof playerEntryData)[]): void {  
    ...
    const adjustPlayersNumBrkts = (): void => {
      ...
      if (totalBrkts.oneBye < 0) {      
        ...
        while (pIndex >= 0) { 
          if (this._brktEntries[pIndex][this._numBrktsName] > totalBrkts.total) {
            break;
          }
          pIndex--;        
        }
        ...
        let newVal = 0;
        while (!done) { 
          while (pIndex >= 0 && this._brktEntries[pIndex][this._numBrktsName] <= totalBrkts.total) {
            pIndex--;          
          }
          if (pIndex >= 0) {
            // new value is min of brktEntRows[pIndex][numBrktsName] and totalBrkts.total
            if (newVal === 0) {
              newVal = Math.min(newNumBrkts, this._brktEntries[pIndex][this._numBrktsName] - 1);
            }              
          ...
          if (pIndex === 0 && this._brktEntries[0][this._numBrktsName] === totalBrkts.total) {
            done = true;
            if (totalBrkts.oneBye < 0) { 
              totalBrkts.full = totalBrkts.full + totalBrkts.oneBye;
              totalBrkts.oneBye = 0;
    ...
    const calculateNumBrackets = (): void => {      
      ...
      totalEntries = this._brktEntries.reduce((total, brktEntry) => total + brktEntry[this._numBrktsName], 0);

      if (this._brktEntries.length === this.playersPerBrkt - 1) {        
        // set one bye to min to min of # brackets entered
        totalBrkts.oneBye = this._brktEntries.reduce((min, brktEntry) => Math.min(min, brktEntry[this._numBrktsName]), Infinity);
      } else { 
        let gotAnswer = false;      
        let addToFull = 1;    
        ...
        if (emptySpots >= 0) {
          ...
          if (this.playersPerBrkt > this._brktEntries.length) {
            ...
            while (b < this._brktCounts.forFullValues.length && remaining > 0) {
              const f = (remaining > this.playersPerBrkt) ? 0 : this.playersPerBrkt - remaining;
              this._brktCounts.forFullValues[b] = f
              this._brktCounts.forOneByeValues[b] = (f === 0) ? 0 : f - 1;
    ...
    const validBrktEntries = (): boolean => { 
      ...
      while (b < this._brktEntries.length) {
        if (!this._brktEntries[b].player_id
    ...
    // 2) remove all rows with no bracket entries
    this._brktEntries = cloneDeep(playerEntries.filter(entry => entry[this._numBrktsName] > 0));
  ...
  canRandomize(): boolean {
     ...
     */
    const validToFillArray = (toFillArr: (number | null)[], filledValue: number): boolean => {
      ...
      for (let num of toFillArr) {
        ...
        } else if (filledValue === 0 && (num === 0 || num === 1) && hasNoEntriesValue) {
          return false;
    ...
    if (this._brktCounts.forFullValues.length === 0 || this._brktCounts.forOneByeValues.length === 0) {
  ...
  createOppoMapForTesting(
    ...
    for (let p = playerStartingIndex; p < this._brktEntries.length; p++) { 
      ...
      oppoMapValue = (Number.isInteger(oppoMapValue)) ? oppoMapValue : Math.ceil(oppoMapValue);
    ...
    if (sumOfValues < playerNumBrkts) {      
      ...
      const uniqueValues = Array.from(new Set(this._brktEntries.map(brktEntry => brktEntry[this._numBrktsName])));        
      let i = 0;
      while (sumOfValues < playerNumBrkts && i < uniqueValues.length) {
        const filteredPlayers = this._brktEntries.filter(brktEntry => brktEntry[this._numBrktsName] === uniqueValues[i]);
  ...
  findPlayer(playerId: string, startBrktIndex: number = 0): findPlayerResult {
    ...
    for (let i = startBrktIndex; i < this.brackets.length; i++) {
      const pIndex = this.brackets[i].playerIndex(playerId);
      if (pIndex >= 0) {
  ...
  putPlayerInBrkt(playerId: string): Bracket | null {
    for (let i = 0; i < this.brackets.length; i++) {
      ...
      if (!brkt.isFull) {
        if (brkt.playerIndex(playerId) === -1) {
...
// END OF RANDOMIZE TEST CODE
  ...
  randomize(forTesting: playerUsedType[]): void {
     ...
     * @param {number} playerNumBrkts - number of brackets player need to be filled
     * @returns {number} - noError or errCantCreateOppoMap
     */
    const createOppoMap = (playerIndex: number, playerNumBrkts: number): number => {
      ...
      for (let p = playerStartingIndex; p < this._brktEntries.length; p++) { 
        ...
        // round up to nearest integer        
        oppoMapValue = (Number.isInteger(oppoMapValue)) ? oppoMapValue : Math.ceil(oppoMapValue);
        // if player has more than 1 bracket, and opponent has less than dupThreshold, set to 1
        if (oppoMapValue > 1 && oppoEntered < this._dupThreshold) { 
          oppoMapValue = 1;
        }  
        oppoMap.set(this._brktEntries[p].player_id, oppoMapValue);  
      ...
      let sumOfValues = values.reduce((sum, value) => sum + value, 0);      
      if (sumOfValues === 0) {
        return BracketList.errNoValuesInOppoMap;
      }
      // if not enough entries in the oppo map, then add entries to the map
      if (sumOfValues < playerNumBrkts) {        
        const max = Math.max(...values);
        // get unique values in the map
        // because oppomap is created from the brktEntries,
        // the uniqueValues array will be sorted in decending order
        const uniqueValues = Array.from(new Set(this._brktEntries.map(brktEntry => brktEntry[this._numBrktsName])));        
        let i = 0;
     ...
     */
    const getAvailBrktIdxesForPlayer = (id: string, includeFullBrkts: boolean): number[] => {
      const playerSet = playersBrktsMap.get(id);
      if (!playerSet) return [];
      const allBrktIndexs = Array.from({ length: this._brackets.length }, (_, index) => index);
      if (!includeFullBrkts) {
        return allBrktIndexs.filter(brktIndex => !playerSet.has(brktIndex) && this._brackets[brktIndex].players.length < this.playersPerBrkt);
      }
      return allBrktIndexs.filter(brktIndex => !playerSet.has(brktIndex));    
    }
    ...
    const getRandomBracketIndexForMatch = (oppoId: string): number => {
      ...
      for (let i = 0; i < shuffledBrktsForPlayer.length; i++) {
        ...
        if (isBrktIdxAvailForPlayer(sbpIndex, playerId)
    ...
    const getSwapIndexesForMatch = (oppoId: string): number[] => {
      ...
      while (cmIndex < fromIndexes.length) { 
        ...
        const swapOppoId = this._brackets[sfIndex].players[this._brackets[sfIndex].players.length - 1];  
        // step 5 - get avaliable brackets for swapOppoId
        const swapOppoAvail = getAvailBrktIdxesForPlayer(swapOppoId, noFullBrkts);
        if (!swapOppoAvail || swapOppoAvail.length === 0) {
          cmIndex++;
        } else { 
          // step 6 - get available brackets for playerId
    ...
    const gotByeEntries = (): boolean => {       
      const byeEntry = this._brktEntries.filter((entry) => entry.first_name === 'Bye' && entry.last_name === 'Bye');
    ...
    while (!doneRandomizing && tryCount <= maxTries) {    
      ...
      outerLoop: {
        ...
        for (let i = 0; i < this._brktEntries.length; i++) {
          neededCountMap.set(this._brktEntries[i].player_id, this._brktEntries[i][this.numBrktsName]);
          playersBrktsMap.set(this._brktEntries[i].player_id, new Set<number>());
        }
        ...
        for (let p = 0; p < this._brktEntries.length - 1; p++) {
          ...
          if (opppoMapErr !== BracketList.noError) { 
            ...
            if (opppoMapErr === BracketList.errCantCreateOppoMap) {
              ...
              errMsg = "Not enough entries in opposition map for player: " + playerId;
            }
          ...
          for (let b = 0; b < playerNumBrkts; b++) {
            ...
            while (needMatch) {
              ...
              if (matchTestResult === matchTestCodes.Valid) {
                const bIndex = putMatchInBracket(this._shuffled[sIndex].playerId);
                // if bracket index is invalid
                if (bIndex < 0) {
                  if (bIndex === BracketList.reRandomize) {
                    const randError: randmoizeErrorType = {
                      error: BracketList.reRandomize,
                      message: "Need to re-randomize, stopped at player: " + playerId,
                      badEntries: this.getDebugShuffle(),
                      badBrackets: getDebugBracketString(),
                      tryCount: tryCount,
                      playerId: playerId,
              ...
              if (p === (this._brktEntries.length - 2)
                && b === (playerNumBrkts - 1)
                && !needMatch
                && currentNumMatches() * 2 === this._brackets.length * this.playersPerBrkt) 
              { 
    ...
    if (this._randomizeErrors[lastErrorIndex].error != BracketList.noError) {
      console.log('doneRandomizing: ', doneRandomizing, '; tryCount: ', tryCount);
  ...
  //   { 
  //     this._errorMessage = 'Invalid bracket counts';
  //     return false;
  //   }
  //   let oneByeCount = 0
  ...
  //       this._errorMessage = `Bracket ${i + 1} count is missing`;
  //       return false;
  //     } 
  //     const count = this._brktCounts.forFullValues[i] as number;
  //     if (count > 1) { 
  //       this._errorMessage = `Bracket ${i + 1} has more than 1 empty spot`;
  //       return false;        
  //     }
  //     if (count === 1) {
  //       oneByeCount++;
  ...
  //       this._errorMessage = `More than ${this.playersPerBrkt - 1} brackets with one bye`;
  //       return false;
  //     }
  //   }
  //   return true;
