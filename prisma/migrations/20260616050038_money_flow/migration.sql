/*
  ...
  - Added the required column `flow` to the `Money` table without a default value. This is not possible if the table is not empty.

*/
-- CreateEnum
CREATE TYPE "MoneyFlow" AS ENUM ('IN', 'OUT');

-- AlterEnum
ALTER TYPE "MoneyDescrip" ADD VALUE 'REFUNDS';
...
-- AlterTable
ALTER TABLE "Money" ADD COLUMN     "flow" "MoneyFlow" NOT NULL,
ALTER COLUMN "id" SET DEFAULT concat('pri_', replace(cast(gen_random_uuid() as text), '-', ''));
