/*
  ...
  - You are about to drop the column `fee` on the `Payout` table. All the data in the column will be lost.
  ...
  - Added the required column `player_id` to the `Payout` table without a default value. This is not possible if the table is not empty.
  - Added the required column `position` to the `Payout` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "Payout" DROP COLUMN "fee",
ADD COLUMN     "amount" DECIMAL(9,2) NOT NULL,
ADD COLUMN     "player_id" TEXT NOT NULL,
ADD COLUMN     "position" INTEGER NOT NULL,
ALTER COLUMN "id" SET DEFAULT concat('pay_', replace(cast(gen_random_uuid() as text), '-', ''));
...
-- CreateTable
CREATE TABLE "Prize_Fund" (
    "id" TEXT NOT NULL DEFAULT concat('prf_', replace(cast(gen_random_uuid() as text), '-', '')),
    ...
    CONSTRAINT "Prize_Fund_pkey" PRIMARY KEY ("id")
);

-- AddForeignKey
ALTER TABLE "Prize_Fund" ADD CONSTRAINT "Prize_Fund_money_id_fkey" FOREIGN KEY ("money_id") REFERENCES "Money"("id") ON DELETE CASCADE ON UPDATE NO ACTION;

-- AddForeignKey
ALTER TABLE "Payout" ADD CONSTRAINT "Payout_player_id_fkey" FOREIGN KEY ("player_id") REFERENCES "Player"("id") ON DELETE CASCADE ON UPDATE NO ACTION;
