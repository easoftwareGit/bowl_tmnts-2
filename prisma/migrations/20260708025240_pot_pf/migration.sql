/*
  ...
  - You are about to drop the column `game_num` on the `Pot_PF` table. All the data in the column will be lost.
  - A unique constraint covering the columns `[pot_id,position]` on the table `Pot_PF` will be added. If there are existing duplicate values, this will fail.

*/
-- DropIndex
DROP INDEX "Pot_PF_pot_id_game_num_position_key";
...
-- AlterTable
ALTER TABLE "Div_PF" ALTER COLUMN "id" SET DEFAULT concat('dpf_', replace(cast(gen_random_uuid() as text), '-', ''));
...
-- AlterTable
ALTER TABLE "Elim_PF" ALTER COLUMN "id" SET DEFAULT concat('epf_', replace(cast(gen_random_uuid() as text), '-', ''));
...
-- AlterTable
ALTER TABLE "Pot_PF" DROP COLUMN "game_num",
ALTER COLUMN "id" SET DEFAULT concat('ppf_', replace(cast(gen_random_uuid() as text), '-', ''));
...
-- CreateIndex
CREATE UNIQUE INDEX "Pot_PF_pot_id_position_key" ON "Pot_PF"("pot_id", "position");
