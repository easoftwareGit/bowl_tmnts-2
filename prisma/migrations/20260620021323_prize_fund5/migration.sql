/*
  ...
  - Added the required column `game_num` to the `Prize_Fund` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "Prize_Fund" ADD COLUMN     "game_num" INTEGER NOT NULL,
...
-- Event prize fund:
-- one row per event + position
CREATE UNIQUE INDEX "Prize_Fund_event_position_unique"
ON "Prize_Fund" ("event_id", "position")
WHERE "event_id" IS NOT NULL;

-- Pot prize fund:
-- one row per pot + position + game_num
CREATE UNIQUE INDEX "Prize_Fund_pot_position_game_unique"
ON "Prize_Fund" ("pot_id", "position", "game_num")
WHERE "pot_id" IS NOT NULL;

-- Elim prize fund:
-- one row per elim + position
CREATE UNIQUE INDEX "Prize_Fund_elim_position_unique"
ON "Prize_Fund" ("elim_id", "position")
WHERE "elim_id" IS NOT NULL;