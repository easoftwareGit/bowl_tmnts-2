...
-- AlterTable
ALTER TABLE "Pot_PF" ALTER COLUMN "id" SET DEFAULT concat('ppf_', replace(cast(gen_random_uuid() as text), '-', ''));
...
-- CreateTable
CREATE TABLE "Div_PF" (
    "id" TEXT NOT NULL DEFAULT concat('dpf_', replace(cast(gen_random_uuid() as text), '-', '')),
    "div_id" TEXT,
    ...
    CONSTRAINT "Div_PF_pkey" PRIMARY KEY ("id")
);

-- CreateIndex
CREATE UNIQUE INDEX "Div_PF_div_id_position_key" ON "Div_PF"("div_id", "position");

-- AddForeignKey
ALTER TABLE "Div_PF" ADD CONSTRAINT "Div_PF_div_id_fkey" FOREIGN KEY ("div_id") REFERENCES "Div"("id") ON DELETE CASCADE ON UPDATE NO ACTION;
