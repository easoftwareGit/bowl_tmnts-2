...
describe('DivPfs - GETs and POST API: /api/divPfs', () => {
  ...
  describe('PUT many divPfs API: /api/divPfs/divPf/:id', () => {
    ...
    it('should update many divPfs for a div - change amount', async () => {
      ...
      const difPfJSON = JSON.stringify(pmDivPfs);
      const response = await privateApi.put(divUrl + pmDivId, difPfJSON);
      expect(response.status).toBe(200);
      putMany = true;
    ...
    it('should NOT update many divPfs for a div when id is not found', async () => { 
      const invalidDivPfs = cloneDeep([pmDivPf1, pmDivPf2]);
      invalidDivPfs[0].div_id = notFoundDivId;
      invalidDivPfs[1].div_id = notFoundDivId;
      const invalidJSON = JSON.stringify(invalidDivPfs);
      try {
        const response = await privateApi.put(divUrl + notFoundDivId, invalidJSON);
    ...
    it('should NOT update many divPfs for a div when id is invalid', async () => { 
      const invalidDivPfs = cloneDeep([pmDivPf1, pmDivPf2]);
      invalidDivPfs[1].id = "test";
      const invalidJSON = JSON.stringify(invalidDivPfs);
      try {
        const response = await privateApi.put(divUrl + pmDivId, invalidJSON);
        expect(response.status).toBe(422);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(422);
    ...
    it('should NOT update many divPfs for a div when all div id are not the same', async () => {       
      const pmOtherDivId = "div_99a3cae28786485bb7a036935f0f6a0a"; // valid and found (from seeds.ts)
      const invalidDivPfs = cloneDeep([pmDivPf1, pmDivPf2]);
      invalidDivPfs[1].div_id = pmOtherDivId;
      const invalidJSON = JSON.stringify(invalidDivPfs);
      try {
        const response = await privateApi.put(divUrl + pmDivId, invalidJSON);
        expect(response.status).toBe(422);
    ...
    it('should NOT update many divPfs for a div when position is too high', async () => { 
      const invalidDivPfs = cloneDeep([pmDivPf1, pmDivPf2]);      
      invalidDivPfs[1].position = maxPosition + 1;
      const invalidJSON = JSON.stringify(invalidDivPfs);
      try {
        const response = await privateApi.put(divUrl + pmDivId, invalidJSON);
        expect(response.status).toBe(422);
    ...
    it('should NOT update many divPfs for a div when position is not a number', async () => { 
      const invalidDivPfs = cloneDeep([pmDivPf1, pmDivPf2]);      
      invalidDivPfs[1].position = "test" as any;
      const invalidJSON = JSON.stringify(invalidDivPfs);
      try {
        const response = await privateApi.put(divUrl + pmDivId, invalidJSON);
        expect(response.status).toBe(422);
    ...
    it('should NOT update many divPfs for a div when position is not in squence', async () => { 
      const invalidDivPfs = cloneDeep([pmDivPf1, pmDivPf2]);      
      invalidDivPfs[1].position = 4;
      const invalidJSON = JSON.stringify(invalidDivPfs);
      try {
        const response = await privateApi.put(divUrl + pmDivId, invalidJSON);
        expect(response.status).toBe(422);
    ...
    it('should NOT update many divPfs for a div when position is missing', async () => { 
      const invalidDivPfs = cloneDeep([pmDivPf1, pmDivPf2]);      
      invalidDivPfs[1].position = null as any;
      const invalidJSON = JSON.stringify(invalidDivPfs);
      try {
        const response = await privateApi.put(divUrl + pmDivId, invalidJSON);
        expect(response.status).toBe(422);
    ...
    it('should NOT update many divPfs for a div when amount is too high', async () => { 
      const invalidDivPfs = cloneDeep([pmDivPf1, pmDivPf2]);      
      invalidDivPfs[1].amount = maxMoney + 1;
      const invalidJSON = JSON.stringify(invalidDivPfs);
      try {
        const response = await privateApi.put(divUrl + pmDivId, invalidJSON);
        expect(response.status).toBe(422);
    ...
    it('should NOT update many divPfs for a div when amount is not a number', async () => { 
      const invalidDivPfs = cloneDeep([pmDivPf1, pmDivPf2]);      
      invalidDivPfs[1].amount = "test" as any;
      const invalidJSON = JSON.stringify(invalidDivPfs);
      try {
        const response = await privateApi.put(divUrl + pmDivId, invalidJSON);
        expect(response.status).toBe(422);
    ...
    it('should NOT update many divPfs for a div when amount is missing', async () => { 
      const invalidDivPfs = cloneDeep([pmDivPf1, pmDivPf2]);      
      invalidDivPfs[1].amount = null as any;
      const invalidJSON = JSON.stringify(invalidDivPfs);
      try {
        const response = await privateApi.put(divUrl + pmDivId, invalidJSON);
        expect(response.status).toBe(422);
  ...
  describe('DELETE by ID - API: /api/divPfs/divPf/:id', () => { 
  ...
  describe('DELETE by by div_id, all divPfs for a div - API: /api/divPfs/divPf/:id', () => { 
    ...
    const restoreDivPfs = async () => {
      await privateApi.delete(divUrl + delDivId);      
      const del1JSON = JSON.stringify(delDivPf1);
      await privateApi.post(url, del1JSON);
      const del2JSON = JSON.stringify(delDivPf2);
    ...
    it('should delete all divPfs for a div by div_id', async () => {
      const response = await privateApi.delete(divUrl + delDivId);
      expect(response.status).toBe(200);
      expect(response.data.count).toBe(2);
      didDel = true;
    })
    it('should NOT delete all divPfs for a div by div_id when div_id is valid, but not found', async () => {
      const response = await privateApi.delete(divUrl + notFoundDivId);
      expect(response.status).toBe(200);
      expect(response.data.count).toBe(0);
    })    
    it('should NOT delete all divPfs for a div by div_id when div_id is invalid', async () => {
      try {
        const response = await privateApi.delete(divUrl + 'test');
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should NOT delete all divPfs for a div by div_id when div_id is valid, but not a div ID', async () => {
      try {        
        const response = await privateApi.delete(divUrl + userId);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
