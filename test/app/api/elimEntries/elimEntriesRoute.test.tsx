...
describe("ElimEntries - API's: /api/elimEntries", () => { 
  ...
  describe('GET one elimEntry API: /api/elimEntries/elimEntry/:id', () => {
    ...
    it('should not get one elimEntry by ID when id is invalid', async () => {
      try {
        const response = await privateApi.get(oneElimEntryUrl + 'test');
    ...
    it('should not get one elimEntry by ID when id is valid, but not a elimEntry id', async () => {
      try {
        const response = await privateApi.get(oneElimEntryUrl + userId);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should not get one elimEntry by ID when id is not found', async () => {
      try {
        const response = await privateApi.get(oneElimEntryUrl + notFoundId);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
  ...
  describe('GET all elimEntries for one div API: /api/elimEntries/div/:divId', () => {
    ...
    it('should get all elimEntries for one div', async () => {
      const response = await privateApi.get(divUrl + divIdForElimEntries);
      expect(response.status).toBe(200);
  ...
  describe('GET all elimEntries for one squad API: /api/elimEntries/squad/:squadId', () => {
    ...
    it('should get all elimEntries for one squad', async () => {
      const response = await privateApi.get(squadUrl + squadIdForElimEntries);
      expect(response.status).toBe(200);
      const elimEntries = response.data.elimEntries;
  ...
  describe('GET all elimEntries for one brkt API: /api/elimEntries/brkt/:brktId', () => {
    ...
    it('should get all elimEntries for one brkt', async () => {
      const response = await privateApi.get(elimUrl + elimIdForElimEntries);
      expect(response.status).toBe(200);
      const elimEntries = response.data.elimEntries;
    ...
    it('should not get all elimEntries for one brkt when brktId is invalid', async () => {
      try {
        const response = await privateApi.get(elimUrl + 'test');
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
  ...
  describe('GET all elimEntries for one tmnt API: /api/elimEntries/tmnt/:tmntId', () => {
    ...
    it('should get all elimEntries for one tmnt', async () => {
      const response = await privateApi.get(tmntUrl + tmntIdForElimEntries);
      expect(response.status).toBe(200);
      const elimEntries = response.data.elimEntries;
  ...
  describe('POST one elimEntry API: /api/elimEntries', () => {
    ...
    it('should post one elimEntry', async () => {
      ...
      const response = await privateApi.post(url, elimEntryJSON);
  ...
  describe('PUT one elimEntry API: /api/elimEntries/elimEntry/:id', () => { 
    ...
    const resetElimEntry = async () => {
      ...
      await privateApi.put(oneElimEntryUrl + testElimEntry.id, elimEntryJSON);
    }
    ...
    it('should update a elimEntry by ID', async () => {
      ...
      const response = await privateApi.put(oneElimEntryUrl + testElimEntry.id, elimEntryJSON);
      expect(response.status).toBe(200);
      didPut = true;
    ...
    it('should not update a elimEntry by ID when elim_id is blank', async () => {
      ...
      const invalidJSON = JSON.stringify(invalidElimEntry);
      try {
        const response = await privateApi.put(oneElimEntryUrl + testElimEntry.id, invalidJSON);
        expect(response.status).toBe(422);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(422);
  ...
  describe('PATCH one elimEntry API: /api/elimEntries/elimEntry/:id', () => { 
    ...
    const toPatchId = 'een_19f158c6cc0d4f619227fbc24a885bab';
    const toPatchElimId = 'elm_c47a4ec07f824b0e93169ae78e8b4b1e'
    ...
    const resetElimEntryPatch = async () => {
      try {
        const playerJSON = JSON.stringify(testElimEntry);
        await privateApi.put(oneElimEntryUrl + testElimEntry.id, playerJSON)
      } catch (err) {
        if (err instanceof AxiosError) console.log(err.message);
      }
    }
    ...
    it('should patch elim_id in a elimEntry by ID', async () => {       
      const patchElimEntry = {        
        elim_id: toPatchElimId,
      }
      const elimEntryJSON = JSON.stringify(patchElimEntry);
      const response = await privateApi.patch(oneElimEntryUrl + toPatchId, elimEntryJSON);
      expect(response.status).toBe(200);
      didPatch = true;
      const patchedElimEntry = response.data.elimEntry;
      expect(patchedElimEntry.id).toBe(toPatch.id);
      expect(patchedElimEntry.elim_id).toBe(toPatchElimId);      
    })
    it('should patch player_id in a elimEntry by ID', async () => { 
      const toPatchPlayerId = 'ply_88be0472be3d476ea1caa99dd05953fa'
      const patchElimEntry = {        
        player_id: toPatchPlayerId,
      }
      const elimEntryJSON = JSON.stringify(patchElimEntry);
      const response = await privateApi.patch(oneElimEntryUrl + toPatchId, elimEntryJSON);
      expect(response.status).toBe(200);
      ...
      const patchedElimEntry = response.data.elimEntry;
      expect(patchedElimEntry.id).toBe(toPatch.id);
      expect(patchedElimEntry.player_id).toBe(toPatchPlayerId);      
    })      
    it('should patch fee in a elimEntry by ID', async () => { 
      ...
      const patchedElimEntry = response.data.elimEntry;
      expect(patchedElimEntry.id).toBe(toPatch.id);
      expect(patchedElimEntry.fee).toBe(toPatchFee);      
    })

    it('should not patch elimEntry by ID when just passing in ID', async () => {
      try {
        const invalidJSON = JSON.stringify({          
          id: toPatchId,
        })
        const response = await privateApi.patch(oneElimEntryUrl + toPatchId, invalidJSON)
    ...
    it('should not patch elimEntry by ID when ID is invalid', async () => {
      try {
        const invalidJSON = JSON.stringify({          
          elim_id: toPatchElimId,
        })
        const response = await privateApi.patch(oneElimEntryUrl + 'test', invalidJSON)
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should not patch elimEntry by ID when ID is valid, but not found', async () => {
      try {
        const invalidJSON = JSON.stringify({          
          elim_id: toPatchElimId,
        })
        const response = await privateApi.patch(oneElimEntryUrl + notFoundId, invalidJSON)
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should not patch elimEntry by ID when ID is valid, but not a money id', async () => {
      try {
        const invalidJSON = JSON.stringify({          
          elim_id: toPatchElimId,
        })
        const response = await privateApi.patch(oneElimEntryUrl + userId, invalidJSON)
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
  ...
  describe('DELETE by ID - API: /api/elimEntries/elimEntry/:id', () => { 
    ...
    it('should delete a elimEntry by ID', async () => { 
      const response = await privateApi.delete(oneElimEntryUrl + toDelElimEntry.id);
      expect(response.status).toBe(200);
      didDel = true;
    })
    it(('should NOT delete a elimEntry by ID when ID is invalid'), async () => {
      try {
        const response = await privateApi.delete(oneElimEntryUrl + "invalid_id");
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should NOT delete a elimEntry by ID when id is valid, but not a elimEntry id', async () => {
      try {
        const response = await privateApi.delete(oneElimEntryUrl + userId);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should NOT delete a elimEntry by ID when id is not found', async () => {
      const response = await privateApi.delete(oneElimEntryUrl + notFoundId);
      expect(response.status).toBe(200);
