...
describe("Players - API's: /api/players", () => {
  ...
  const deletePostedPlayer = async (playerId: string) => {
    try {
      await privateApi.delete(onePlayerUrl + playerId);
    } catch (err) {
  ...
  describe('GET player by ID - API: /api/players/player/:id', () => { 
    ...
    it('it should get a Bye player', async () => {
      try {
        const response = await privateApi.get(onePlayerUrl + byeId);
        const player = response.data.player;
        expect(response.status).toBe(200);
    ...
    it('should NOT get a player by id when id is invalid', async () => {
      try {
        const response = await privateApi.get(onePlayerUrl + 'test');
    ...
    it('should NOT get a player by id when id is valid, but not a player id', async () => {
      try {
        const response = await privateApi.get(onePlayerUrl + nonPlayerId);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should NOT get a player by id when id is not found', async () => {
      try {
        const response = await privateApi.get(onePlayerUrl + notFoundId);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
  ...
  describe('GET all players for a squad API: /api/players/squad/:squadId', () => {    
    ...
    it('should get all players for a squad', async () => {
      const response = await privateApi.get(squadUrl + squadId);
      expect(response.status).toBe(200);
    ...
    it('should NOT get all players for a squad when squad id is valid, but not a squad id', async () => {
      try {
        const response = await privateApi.get(squadUrl + notFoundTmntId);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
  ...
  describe('GET all players for a tournament API: /api/players/tmnt/:tmntId', () => { 
    ...
    it('should get all players for a tournament', async () => {
      ...
      const response = await privateApi.get(tmntUrl + wholeTmntId);
      expect(response.status).toBe(200);
      const players = response.data.players;
    ...
    it('should NOT get all players for a tournament when tmnt id is valid, but not a tmnt id', async () => {
      try {
        const response = await privateApi.get(tmntUrl + squadId);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
  ...
  describe('PUT by ID - API: /api/players/player/:id', () => {
    ...
    const resetPlayer = async () => {
      ...
      await privateApi.put(onePlayerUrl + testPlayer.id, playerJSON);
    }
    ...
    it('should update a player by ID', async () => {
      ...
      const response = await privateApi.put(onePlayerUrl + testPlayer.id, playerJSON);
      expect(response.status).toBe(200);
      didPut = true;
    ...
    it('should update a sanitized player by ID', async () => {
      ...
      const response = await privateApi.put(onePlayerUrl + testPlayer.id, sanitizeJSON);
      expect(response.status).toBe(200);
      didPut = true;
      const puttedPlayer = response.data.player;
  ...
  describe('PATCH by ID - API: /api/players/player/:id', () => { 
    ...
    const toPatchId = "ply_91c5aa1c14644e03b6735abd1480ee32";
    ...
    const resetPatched = async () => {
      try {
        const playerJSON = JSON.stringify(toPatch);
        await privateApi.put(onePlayerUrl + toPatchId, playerJSON);
      } catch (err) {
        if (err instanceof AxiosError) console.log(err.message);
      }
    }
    ...
    it('should patch first_name in a player by ID', async () => {
      ...
      const response = await privateApi.patch(onePlayerUrl+ toPatchId, playerJSON);
      expect(response.status).toBe(200);
      didPatch = true;
    ...
    it('should NOT patch a bye player', async () => { 
      try {
        ...
        const invalidJSON = JSON.stringify(byePlayer);
        const response = await privateApi.patch(onePlayerUrl + byePlayerToPost.id, invalidJSON);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should not patch money by ID when just passing in ID', async () => {
      try {
        ...
        const response = await privateApi.patch(onePlayerUrl + toPatchId, invalidJSON)
    ...
    it('should NOT patch when player id is invalid', async () => {
      try {
        ...
        const playerJSON = JSON.stringify(patchPlayer);
        const response = await privateApi.patch(onePlayerUrl + "invalid_id", playerJSON);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should NOT patch when player id is valid, but no a player id', async () => {
      try {
        ...
        const playerJSON = JSON.stringify(patchPlayer);
        const response = await privateApi.patch(onePlayerUrl + nonPlayerId, playerJSON);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should NOT patch when player id is valid but not found', async () => {
      try {
        ...
        const playerJSON = JSON.stringify(patchPlayer);
        const response = await privateApi.patch(onePlayerUrl + notFoundId, playerJSON);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should return 400 when just passing squad_id, no patching done', async () => {
      try {
        const patchPlayer = {          
          squad_id: squadId,
        }
    ...
    it('should not patch when average is too low', async () => {
      try {
        const invalidPlayer = {          
          average: -1
        }
    ...
    it('should not patch when average is too high', async () => {
      try {
        const invalidPlayer = {
          ...toPatch,
          average: maxScore + 1
    ...
    it('should not patch when average is not an integer', async () => {
      try {
        const invalidPlayer = {          
          average: 123.45
        }
    ...
    it('should not patch when average is not a number', async () => {
      try {
        const invalidPlayer = {          
          average: "abc"
        }
    ...
    it('should not patch when lane is too low', async () => {
      try {
        const invalidPlayer = {          
          lane: -1
        }
    ...
    it('should not patch when lane is too high', async () => {
      try {
        const invalidPlayer = {
          ...toPatch,
          lane: maxLaneCount + 1
    ...
    it('should not patch when lane is not an integer', async () => {
      try {
        const invalidPlayer = {          
          lane: 123.45
        }
    ...
    it('should not patch when lane is not a number', async () => {
      try {
        const invalidPlayer = {          
          lane: "abc"
        }
    ...
    it('should not patch when position is blank', async () => {
      try {
        const invalidPlayer = {          
          position: ""
        }
  ...
  describe('DELETE by ID - API: /api/players/player/:id', () => { 
    ...
    it('should delete a player by id', async () => {
      const response = await privateApi.delete(onePlayerUrl + toDelPlayer.id);
      expect(response.status).toBe(200);
      didDel = true;
    })
    it('should delete a bye player by id', async () => {
      try {
        const playerJSON = JSON.stringify(byePlayerToPost);
        const insertResponse = await privateApi.post(url, playerJSON);        
        expect(insertResponse.status).toBe(201);
        didInsertBye = true;
        const response = await privateApi.delete(onePlayerUrl + byePlayerToPost.id);
        expect(response.status).toBe(200);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should NOT delete a player by id when id is not valid', async () => {
      try {
        const response = await privateApi.delete(onePlayerUrl + 'test');;
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should NOT delete a player by id when id is valid, but not a player id', async () => {      
      try {
        const response = await privateApi.delete(onePlayerUrl + nonPlayerId);
        expect(response.status).toBe(404);
      } catch (err) {
        if (err instanceof AxiosError) {
          expect(err.response?.status).toBe(404);
    ...
    it('should return 0 when delete a player by id when id is not found', async () => {
      const response = await privateApi.delete(onePlayerUrl + notFoundId);
      expect(response.status).toBe(200);
