...
} from "@/app/dataEntry/prizeFunds/oldDivPfGrid/sfCreateDivPfColumns";
...
describe("createDivPfColumns", () => {
  ...
  it("creates the expected column definitions", () => {
    ...
    const result = createDivPfColumns(onHandleMoneyCellChange);

    expect(result).toHaveLength(4);

    expect(result[0]).toEqual({
      field: divPfEntryIdColName,
      headerText: "ID",
      isPrimaryKey: true,
      visible: false,
    });
  ...
  it("creates the amount column money editor with the correct settings", () => {
    ...
    createDivPfColumns(onHandleMoneyCellChange);
  ...
  it("uses the same widths on touch devices", () => {
    ...
    const result = createDivPfColumns();
  ...
  it("validates amount values between 0 and maxMoney", () => {
    ...
    const amountCol = result.find(
      (col) => col.field === divPfEntryAmountColName,
    );
