...
import { setup, standardBeforeEach } from "./potPfPage.testSetup.test";
import { potCategoriesTypes } from "@/lib/types/types";

const totalPrizeFundPotTypes = [
  "Last Game",
  "Series",
] as const satisfies readonly potCategoriesTypes[];

describe("Game pot expenses", () => {
  beforeEach(() => {
    standardBeforeEach();
  });

  it("updates the grid per-game prize fund when expenses change", async () => {
    const {
      user,
      tmntData,
      potId,
    ...
    await user.type(expensesInput, "10");
    await user.tab();

    const expected = (entryFees - 10) / games; 
...
describe.each(totalPrizeFundPotTypes)(
  "%s expenses",
  (potType) => {
    beforeEach(() => {
      standardBeforeEach();
    ...
    it("passes the total prize fund to the grid", async () => {
      const {
        user,
        tmntData,
        potId,
...
describe("Expenses entry tests", () => {
  ...
  it("limits expenses to the maximum prize fund", async () => {
    ...
    expect(expenses).toHaveValue(prizeFund.toFixed(2));
  });
