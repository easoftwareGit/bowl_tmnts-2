...
describe("Division Prize Fund web page cashers input", () => {
  ...
  it("adds rows on blur when the cashers value increases", async () => {
    ...
    expect(
      calculatedCashersInput,
    ).toHaveValue(newCashers);

    expect(mockBtDbUuid).toHaveBeenCalledTimes(
      newCashers - initialCashers,
    );

    expect(mockBtDbUuid).toHaveBeenCalledWith(
      "dpf",
    );
    ...
    const addedIds = addedRows.map(
      (row) => row.id,
    );

    expect(new Set(addedIds).size).toBe(
      addedIds.length,
    );    
    ...
    // ).toEqual([
    //   {
    //     id: "dpf_99999999999999999999999999999999",
    //     parent_id: divId1,
    //     position: initialCashers + 1,
    //     amount: 0,
    //     percentage: 0,
