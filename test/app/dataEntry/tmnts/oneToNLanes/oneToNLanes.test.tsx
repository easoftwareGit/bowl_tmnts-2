...
describe("OneToNLanes - Component", () => { 
  ...
  describe('render the tabs', () => { 
    ...
    it("renders one Tab per squad", () => {
      render(<OneToNLanes {...mockOneToNLanesProps} />);

      const tabs = screen.getAllByRole("tab");
      expect(tabs).toHaveLength(mockSquads.length);
    });

    it("renders with no squads without crashing and no tabs", () => {
      render(
        <OneToNLanes
          lanes={[]}
          setLanes={mockSetLanes}
    ...
    it("selects the first squad tab by default", () => {
      render(<OneToNLanes {...mockOneToNLanesProps} />);
    ...
    it("switches to second squad tab on click", async () => {
      const user = userEvent.setup();
      render(<OneToNLanes {...mockOneToNLanesProps} />);
  ...
  describe('Render the component - isDisabled = true', () => { 
    ...
    it("does not call setLanes when isDisabled = true", async () => {
      const user = userEvent.setup();
      const justOneSquadProps = {
        lanes: lanesOneSquad,
        setLanes: mockSetLanes,
      ...
      // Click should effectively do nothing because inputs are disabled
