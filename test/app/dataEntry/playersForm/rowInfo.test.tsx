...
describe("rowInfo tests", () => {
  ...
  const brkt2TimeStartColName = timeStampColName(mockTmntFullData.brkts[1].id);
  ...
  const mockAllBrktsList: Record<string, BracketList> = {
    [mockTmntFullData.brkts[0].id]: new BracketList(
      mockTmntFullData.brkts[0].id,
      2,
      3,
    ),
    [mockTmntFullData.brkts[1].id]: new BracketList(
      mockTmntFullData.brkts[1].id,
      2,
      3,
    ),
  };
  ...
  const eightPlayers = [validRow, row2, row3, row4, row5, row6, row7, row8];
  ...
  describe("validAverage", () => {
    ...
    it('should return "" when average is undefined and checkType is Prelim', () => {
      const invalidRow = { ...invalidBaseRow, average: undefined };
      const result = validAverage(invalidRow, errPlayer, 1, CheckType.PRELIM);
      expect(result).toBe("");
    });
  ...
  describe("validLane", () => {
    ...
    it("returns error when lane is not an integer (e.g. 1.5)", () => {
      const row = { ...playerEntryData, lane: 1.5 };
      const result = validLane(row, 1, 10, errPlayer, 0, CheckType.PRELIM);
      expect(result).toBe("Invalid Lane for Tom Jones in row 1");
    });
    it("returns error when lane is not a number (e.g. '5')", () => {
      const row = { ...playerEntryData, lane: "5" as any };
      const result = validLane(row, 1, 10, errPlayer, 0, CheckType.PRELIM);
      expect(result).toBe("Invalid Lane for Tom Jones in row 1");
    });
  });
  ...
  describe("validDivs", () => {
    it("should return empty error message when fee is valid number within range and checkType is Prelim", () => {
      const vdErrPlayer = validRow.first_name + " " + validRow.last_name;
      const result = validDivs(
        validRow,
        mockTmntFullData,
        vdErrPlayer,
        0,
    ...
    it("returns empty string when division fees are numeric strings", () => {
      // row2 already has strings for div/pot/elim in your setup
      const result = validDivs(
        row2 as any,
        mockTmntFullData,
        "Jane Doe",
  ...
  describe("validPots", () => {
    ...
    it("should return error message with player name when pot 1 fee is invalid", () => {
      ...
      const result = validPots(row, mockTmntFullData, playerDivs, errPlayer, 0);
      expect(result).toBe(
        "Invalid pot fee in Scratch: Gm for Tom Jones in row 1",
      );
    });
    it("should return error message with player name when pot 2 fee is invalid", () => {
      ...
      const result = validPots(row, mockTmntFullData, playerDivs, errPlayer, 0);
      expect(result).toBe(
        "Invalid pot fee in Scratch: LG for Tom Jones in row 1",
      );
    });
    ...
    it("should return error message when player entred in a pot they are not eligible for", () => {
      const row = {
        ...invalidBaseRow,
        [div1FeeColName]: undefined as any,
        [pot1FeeColName]: 20,
        [pot2FeeColName]: 10,
      };
      ...
      expect(result).toBe(
        "Tom Jones is not entered in the division for pot Scratch: Gm in row 1",
      );
    });
  });
  ...
  describe("validBrkts", () => {
    ...
    it("should return empty string when bracket numbers are null", () => {
      const row = {
        ...invalidBaseRow,
        [brkt1NumColName]: null,
        [brkt2NumColName]: null,
      };
      const result = validBrkts(
        row,
        mockTmntFullData,
    ...
    it("should return empty string when bracket numbers are undefined", () => {
      const row = {
        ...invalidBaseRow,
        [brkt1NumColName]: undefined,
        [brkt2NumColName]: undefined,
      };
      const result = validBrkts(
        row,
        mockTmntFullData,
    ...
    it("should return error message when bracket 1 number is negative", () => {
      ...
      expect(result).toBe(
        "Number of brackets in Scratch: 1-3 is less than 0 for Tom Jones in row 1",
      );
    });
    it("should return error message when bracket 2 number is negative", () => {
      ...
      expect(result).toBe(
        "Number of brackets in Scratch: 4-6 is less than 0 for Tom Jones in row 1",
      );
    });
    it("should return error message when bracket 1 number is too large", () => {
      const row = {
        ...invalidBaseRow,
        [brkt1NumColName]: maxBrackets + 1,
        [brkt2NumColName]: 4,
      };
      const result = validBrkts(
        row,
      ...
      expect(result).toBe(
        "Number of brackets in Scratch: 1-3 is more than 999 for Tom Jones in row 1",
      );
    });
    it("should return error message when bracket 2 number is too large", () => {
      ...
      expect(result).toBe(
        "Number of brackets in Scratch: 4-6 is more than 999 for Tom Jones in row 1",
      );
    });
    it("should return error message when player entred in a bracket they are not eligible for", () => {
      ...
      expect(result).toBe(
        "Tom Jones is not entered in the division for bracket Scratch: 1-3 in row 1",
      );
    });
  });
  ...
  describe("validElims", () => {
    ...
    it("should return empty string when elim fee is null or undefined", () => {
      const row = {
        ...invalidBaseRow,
        [elim1FeeColName]: null,
        [elim2FeeColName]: undefined,
      };
      const result = validElims(
        row,
        mockTmntFullData,
    ...
    it("should return error message with player name when elim 1 fee invalid", () => {
      ...
      expect(result).toBe(
        "Invalid eliminator fee in Scratch: 1-3 for Tom Jones in row 1",
      );
    });
    it("should return error message with player name when elim 2 fee invalid", () => {
      ...
      expect(result).toBe(
        "Invalid eliminator fee in Scratch: 4-6 for Tom Jones in row 1",
      );
    });
    ...
    it("should return error message when player entred in an elim they are not eligible for", () => {
      ...
      expect(result).toBe(
        "Tom Jones is not entered in the division for eliminator Scratch: 1-3 in row 1",
      );
    });
  });
  ...
  describe("findNextError - Prelim", () => {
    ...
    it("should return empty string when rows array is empty", () => {
      const emptyRows: playerEntryRow[] = [];
      const result = findNextError(
        emptyRows,
        mockTmntFullData,
        CheckType.PRELIM,
    ...
    it("should return error message when pot fee is invalid", () => {
      ...
      expect(result.msg).toBe(
        "Invalid pot fee in Scratch: Gm for Tom Jones in row 2",
      );
    });
    it("should return error message when 2nd pot fee is invalid", () => {
      ...
      expect(result.msg).toBe(
        "Invalid pot fee in Scratch: LG for Tom Jones in row 2",
      );
    });
    it("should return error message when number of brackets is too low", () => {
      ...
      expect(result.msg).toBe(
        "Number of brackets in Scratch: 1-3 is less than 0 for Tom Jones in row 2",
      );
    });
    it("should return error message when number of brackets is too high", () => {
      ...
      expect(result.msg).toBe(
        "Number of brackets in Scratch: 1-3 is more than 999 for Tom Jones in row 2",
      );
    });
    it("should return error message when eliminator fee is invalid", () => {
      ...
      expect(result.msg).toBe(
        "Invalid eliminator fee in Scratch: 1-3 for Tom Jones in row 2",
      );
    });
    it("should return error message when 2nd pot fee is invalid", () => {
      const invalidRow = {
      ...
      expect(result.msg).toBe(
        "Invalid eliminator fee in Scratch: 4-6 for Tom Jones in row 2",
      );
    });
  });
  ...
  describe("findNextError - missing tournament structure guards", () => {
    it("returns 'No tournament data' when tmntData is null", () => {
      const result = findNextError([validRow], null as any, CheckType.PRELIM);
      expect(result).toEqual({ id: "none", msg: "No tournament data" });
    });
    it("returns 'No tournament data' when tmntData.tmnt is missing", () => {
      const bad = { ...mockTmntFullData, tmnt: undefined as any };
      const result = findNextError([validRow], bad as any, CheckType.PRELIM);
      expect(result).toEqual({ id: "none", msg: "No tournament data" });
    });
    it("returns 'No event in the tournament' when events is empty", () => {
      const bad = { ...mockTmntFullData, events: [] as any[] };
      const result = findNextError([validRow], bad as any, CheckType.PRELIM);
      expect(result).toEqual({ id: "none", msg: "No event in the tournament" });
    });
    it("returns 'No divison in the tournament' when divs is empty", () => {
      const bad = { ...mockTmntFullData, divs: [] as any[] };
      const result = findNextError([validRow], bad as any, CheckType.PRELIM);
      expect(result).toEqual({
        id: "none",
        msg: "No divison in the tournament",
      });
    });
    it("returns 'No squad in the tournament' when squads is empty", () => {
      const bad = { ...mockTmntFullData, squads: [] as any[] };
      const result = findNextError([validRow], bad as any, CheckType.PRELIM);
      expect(result).toEqual({ id: "none", msg: "No squad in the tournament" });
    });
    it("returns 'No lanes in the tournament' when lanes is empty", () => {
      const bad = { ...mockTmntFullData, lanes: [] as any[] };
      const result = findNextError([validRow], bad as any, CheckType.PRELIM);
      expect(result).toEqual({ id: "none", msg: "No lanes in the tournament" });
    });
  });

  describe("findNextError - duplicate name detection and id", () => {
    it("returns duplicate-name error when two rows have the same full name", () => {
      const dupe = {
        ...validRow,
        id: "playerX",
        // same name as validRow
        first_name: validRow.first_name,
        last_name: validRow.last_name,
        position: "B",
        // avoid lane/position collisions not needed; duplication triggers before those validations anyway
      };
      ...
      expect(result.id).toBe("playerX");
      expect(result.msg).toBe(`Duplicate Player Name "John Smith" in row 2`);
    });
    it("returns the row id for the first row that fails validation", () => {
      const bad = {
        ...invalidBaseRow,
        id: "badRowId",
        first_name: "",
      ...
      expect(result.id).toBe("badRowId");
      expect(result.msg).toBe("Missing First Name in row 2");
    });
  });

  describe("findNextError - duplicate name detection with normalization", () => {
    it("flags duplicate names even if casing differs", () => {
      const r1 = {
        ...validRow,
        id: "p1",
        first_name: "John",
    ...
    it("flags duplicate names even if whitespace differs", () => {
      const r1 = {
        ...validRow,
        id: "p1",
        first_name: "John",
    ...
    it("duplicate detection: does NOT collapse internal whitespace (document current behavior)", () => {
      const r1 = {
        ...validRow,
        id: "p1",
        first_name: "John",
      ...
      // If this currently flags duplicate, expect p2 + duplicate msg.
      // If it currently does NOT, expect msg === "".
      expect(result.id).toBe("p2");
      expect(result.msg).toBe(`Duplicate Player Name "John Smith" in row 2`);
    });
  });

  describe("findNextError - lane/position pairing", () => {
    it("flags duplicate lane+position", () => {
      const r1 = { ...validRow, id: "p1", lane: 30, position: "A" };
      const r2 = { ...row2, id: "p2", lane: 30, position: "A" }; // same lane+pos

      const result = findNextError([r1, r2], mockTmntFullData, CheckType.FINAL);
      expect(result.id).toBe("p2");
      expect(result.msg).toBe("Duplicate Lane/Position 30-A in row 2");
    });
    it("does NOT flag lane-only conflicts during PRELIM when missing lane/position is allowed", () => {
      const r1 = { ...validRow, id: "p1", lane: 30, position: "" as any };
      const r2 = { ...row2, id: "p2", lane: 30, position: "" as any };
    ...
    it("FINAL lane/position duplicate is case-insensitive (a vs A)", () => {
      const r1 = { ...validRow, id: "p1", lane: 30, position: "a" };
      const r2 = { ...row2, id: "p2", lane: 30, position: "A" };
  ...
  describe("findNextError - invalid pot entry", () => {
    it("should return error message when entered pot and matching division entry is null", () => {
      const invalidPlayers = cloneDeep(eightPlayers);
      invalidPlayers[0][div1FeeColName] = null as any;
      const res = findNextError(
        invalidPlayers,
        mockTmntFullData,
        CheckType.PRELIM,
      );
      expect(res.id).toBe(invalidPlayers[0].id);
      expect(res.msg).toMatch(/is not entered in the division for pot/i);
    });
    it("should return error message when entered pot and matching division entry is 0", () => {
      const invalidPlayers = cloneDeep(eightPlayers);
      invalidPlayers[0][div1FeeColName] = 0;
      const res = findNextError(
        invalidPlayers,
        mockTmntFullData,
        CheckType.PRELIM,
  ...
  describe("findNextError - invalid bracket entry", () => {
    it("should return error message when entered bracket and matching division entry is null", () => {
      const invalidPlayers = cloneDeep(eightPlayers);
      invalidPlayers[2][div1FeeColName] = null as any; // use row 3, index 2
      invalidPlayers[2][pot1FeeColName] = null as any; // clear pot entry
      invalidPlayers[2][pot2FeeColName] = null as any;
      const res = findNextError(
        invalidPlayers,
        mockTmntFullData,
        CheckType.PRELIM,
      );
      expect(res.id).toBe(invalidPlayers[2].id);
      expect(res.msg).toMatch(/is not entered in the division for bracket/i);
    });
    it("should return error message when entered bracket and matching division entry is 0", () => {
      const invalidPlayers = cloneDeep(eightPlayers);
      invalidPlayers[2][div1FeeColName] = null as any; // use row 3, index 2
      invalidPlayers[2][pot1FeeColName] = null as any;
      invalidPlayers[2][pot2FeeColName] = null as any;
      const res = findNextError(
        invalidPlayers,
        mockTmntFullData,
  ...
  describe("findNextError - invalid elimiator entry", () => {
    it("should return error message when entered eliminator and matching division entry is null", () => {
      const invalidPlayers = cloneDeep(eightPlayers);
      invalidPlayers[2][div1FeeColName] = null as any; // use row 3, index 2
      invalidPlayers[2][pot1FeeColName] = null as any; // clear pot entry
      invalidPlayers[2][pot2FeeColName] = null as any;
      invalidPlayers[2][brkt1FeeColName] = null as any; // clear bracket entry
      invalidPlayers[2][brkt1NumColName] = null as any;
      invalidPlayers[2][brkt1TimeStartColName] = null as any;
      invalidPlayers[2][brkt2FeeColName] = null as any;
      invalidPlayers[2][brkt2NumColName] = null as any;
      invalidPlayers[2][brkt2TimeStartColName] = null as any;
      const res = findNextError(
        invalidPlayers,
        mockTmntFullData,
        CheckType.PRELIM,
    ...
    it("should return error message when entered eliminator and matching division entry is 0", () => {
      const invalidPlayers = cloneDeep(eightPlayers);
      invalidPlayers[2][div1FeeColName] = null as any; // use row 3, index 2
      invalidPlayers[2][pot1FeeColName] = null as any;
      invalidPlayers[2][pot2FeeColName] = null as any;
  ...
  describe("getDivsPotsBrktsElimsCounts", () => {
    it("should return counts of divs, pots, brkts and elims entries", () => {
      const counts = getDivsPotsBrktsElimsCounts(
        mockEntriesCount,
        mockAllBrktsList,
      );
    ...
    it("should return 6 for brackets counts when only 6 players entered", () => {
      ...
      sixOfEightBrkts[mockTmntFullData.brkts[0].id].calcTotalBrkts(
        sixOfEightPlayers,
      );
      sixOfEightBrkts[mockTmntFullData.brkts[1].id].calcTotalBrkts(
        sixOfEightPlayers,
      );
      const counts = getDivsPotsBrktsElimsCounts(
        mockEntriesCount,
  ...
  describe("getDivsPotsBrktsElimsCountErrMsg", () => {
    ...
    it("should return error message when division count is invalid", () => {
      ...
      const result = getDivsPotsBrktsElimsCountErrMsg(
        invalidCounts,
        mockTmntFullData,
      );
    ...
    it("should return error message when bracket count is invalid", () => {
      ...
      const brktName = getBrktOrElimName(
        brkt as brktType,
    ...
    it("should return error message when eliminator count is invalid", () => {
      ...
      const elimName = getBrktOrElimName(
        elim as elimType,
        mockTmntFullData.divs,
      );
  ...
  describe("getDivsPotsBrktsElimsCountErrMsg - unknown-id branches and boundary", () => {
    it("returns 'unknown division' when div id is not found in tmntData.divs", () => {
      const counts = { ["div_unknown" + feeColNameEnd]: 0 } as Record<
        string,
        number
      >;
      const result = getDivsPotsBrktsElimsCountErrMsg(counts, mockTmntFullData);
      expect(result).toBe(
        "No division entries for unknown division div_unknown",
      );
    });
    it("returns 'unknown pot' when pot id is not found in tmntData.pots", () => {
      const counts = { ["pot_unknown" + feeColNameEnd]: 0 } as Record<
        string,
        number
      >;
      const result = getDivsPotsBrktsElimsCountErrMsg(counts, mockTmntFullData);
      expect(result).toBe("No pot entries for unknown pot pot_unknown");
    });
    it("returns 'unknown elim' when elim id is not found in tmntData.elims", () => {
      const counts = { ["elm_unknown" + feeColNameEnd]: 0 } as Record<
        string,
        number
      >;
      const result = getDivsPotsBrktsElimsCountErrMsg(counts, mockTmntFullData);
      expect(result).toBe("No elim entries for unknown elim elm_unknown");
    });
    it("returns 'unknown bracket' when bracket id is not found in tmntData.brkts", () => {
      const counts = {
        ["brk_unknown" + brktsColNameEnd]: defaultBrktPlayers - 2,
      } as Record<string, number>;
      const result = getDivsPotsBrktsElimsCountErrMsg(counts, mockTmntFullData);
      expect(result).toBe(
        "Not enough bracket entries for unknown bracket brk_unknown",
      );
    });
    it("does NOT error when bracket count is exactly the minimum valid (defaultBrktPlayers - 1)", () => {
      const counts = { [brkt1NumColName]: defaultBrktPlayers - 1 } as Record<
        string,
        number
      >;
      const result = getDivsPotsBrktsElimsCountErrMsg(counts, mockTmntFullData);
