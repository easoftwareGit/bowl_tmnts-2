...
const rePostStage = async (stageToRepost: fullStageType) => {
  try {
    const response = await publicApi.get(`${oneStageUrl}${stageToRepost.id}`);
...
describe("dbStages", () => {
  describe("getFullStageForSquad()", () => {
    ...
    it("should return a stage", async () => {
      ...
      const stageSetAtMs = new Date(fullStage.stage_set_at).getTime();
      ...
      expect(fullStage.stage_override_enabled).toBe(
        testStage.stage_override_enabled
      );
      expect(fullStage.stage_override_at).toEqual(testStage.stage_override_at);
      expect(fullStage.stage_override_reason).toBe(
        testStage.stage_override_reason
      );
    });
    ...
    it("should throw error if squad id does not exist", async () => {
      try {
        ...
        expect((err as Error).message).toBe(
          "getFullStageForSquad failed: Request failed with status code 404"
        );
      }
    });
  ...
  describe("getJustStage()", () => {
    ...
    it("should return justStage when passed stage id ", async () => {
      ...
      const stageSetAtMs = new Date(justStage.stage_set_at).getTime();
    ...
    it("should throw error if stage id does not exist", async () => {
      try {
        ...
        expect(err).toBeInstanceOf(Error);
        expect((err as Error).message).toBe(
          "getJustStage failed: Invalid squad id"
        );
      }
    });
  ...
  describe("getJustStageOverride()", () => {
    ...
    it("should return justStageOverride when passed squadId", async () => {
      ...
      expect(justStage.stage_override_enabled).toBe(
        testStage.stage_override_enabled
      );
      expect(justStage.stage_override_at).toBe(testStage.stage_override_at);
      expect(justStage.stage_override_reason).toBe(
        testStage.stage_override_reason
      );
    });
    ...
    it("should get stage override for squad with override data", async () => {
      ...
      expect(justStage.stage_override_at).not.toBeNull();
      if (justStage.stage_override_at == null) return;
      const stageOverrideAtMs = new Date(justStage.stage_override_at).getTime();

      expect(justStage.id).toBe(testOverrideStage.id);
      expect(justStage.squad_id).toBe(testOverrideStage.squad_id);
      expect(justStage.stage_override_enabled).toBe(
        testOverrideStage.stage_override_enabled
      );
      ...
      expect(justStage.stage_override_reason).toBe(
        testOverrideStage.stage_override_reason
      );
    });
    ...
    it("should throw error if squad id does not exist", async () => {
      try {
        ...
        expect(err).toBeInstanceOf(Error);
        expect((err as Error).message).toBe(
          "getJustStageOverride failed: getFullStageForSquad failed: Request failed with status code 404"
        );
      }
    });
    ...
    it("should throw error if squad id is invalid", async () => {
      try {
        ...
        expect(err).toBeInstanceOf(Error);
        expect((err as Error).message).toBe(
          "getJustStageOverride failed: Invalid squad id"
        );
      }
    });
  ...
  describe("postFullStage()", () => {
    ...
    it("should post a full stage", async () => {
      ...
      expect(postedStage.stage_override_enabled).toBe(
        mockStageToPost.stage_override_enabled
      );
      expect(postedStage.stage_override_at).toBe(mockStageToPost.stage_override_at);
      expect(postedStage.stage_override_reason).toBe(
        mockStageToPost.stage_override_reason
      );
    });
    ...
    it("should post a full stage - all fields", async () => {
      ...
      expect(postedStage.stage_override_enabled).toBe(
        allStageFields.stage_override_enabled
      );
      ...
      expect(postedStage.stage_override_reason).toBe(
        allStageFields.stage_override_reason
      );
    });
    ...
    it("should post a full stage - all fields and ignore dates passed in. system sets dates", async () => {
      ...
      expect(postedStage.stage_override_enabled).toBe(
        ignoreDatesStage.stage_override_enabled
      );
      ...
      expect(postedStage.stage_override_reason).toBe(
        ignoreDatesStage.stage_override_reason
      );
    });
    ...
    it("should post a full stage when sanitized", async () => {
      ...
      const sanitizedStage: fullStageType = {
        ...
        stage: SquadStage.SCORES,
        stage_override_enabled: true,
        stage_override_reason: "<sccript>   test  reason   </script>",
      };
      ...
      expect(postedStage.stage_override_enabled).toBe(
        sanitizedStage.stage_override_enabled
      );
  ...
  describe("postInitialStageForSquad()", () => {
    ...
    afterEach(async () => {
      if (didPost && postedStageId) await deletePostedStage(postedStageId);
    });
    ...
    it("should post an initial full stage", async () => {
      ...
      expect(initialStage.stage_override_reason).toBe(
        mockStageToPost.stage_override_reason
      );
    });
  ...
  describe("patchJustStageOverride()", () => {
    describe("patchJustStageOverride() - setting stage_override_enabled to true", () => {
      ...
      it("should patch a justStageOverride", async () => {
        const before = new Date();
        const patchedStage = await patchJustStageOverride(
          testStage.id,
          true,
          testReason
        );
        ...
        const stageOverrideAtMs = new Date(
          patchedStage.stage_override_at!
        ).getTime();
        ...
        expect(stageOverrideAtMs).toBeGreaterThanOrEqual(
          before.getTime() - twoMinutes
        );
        expect(stageOverrideAtMs).toBeLessThanOrEqual(
          after.getTime() + twoMinutes
        );
      ...
      it("should not patch a justStageOverride when enabled = true, but reason is invalid", async () => {
        try {
          await patchJustStageOverride(
            testStage.id,
            true,
            "a".repeat(maxReasonLength + 1)
          );
        } catch (err) {
          expect(err).toBeInstanceOf(Error);
          expect((err as Error).message).toBe("Invalid justStageOverride data");
