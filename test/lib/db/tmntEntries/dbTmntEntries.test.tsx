...
describe('dbTmntEntries', () => { 
  ...
  describe('updateCurrentElimEntries', () => {
    ...
    it('should return updated elimEntries when elimEntries deleted', () => { 
      const editedElimEntries = cloneDeep(mockOrigData.elimEntries).slice(0, 2)
      const mockAllEnts = cloneDeep(mockAllEntrisOneSquad)
      const toSave = getElimEntriesToSave(editedElimEntries, mockOrigData.elimEntries);
      expect(toSave.length).toBe(2);
      ...
      const updatedElimEntries = updateCurrentElimEntries(toSave, mockAllEnts);
      expect(updatedElimEntries).not.toBe(null)
      if (updatedElimEntries === null) return;
      expect(updatedElimEntries).toHaveLength(2);
      let deleted = updatedElimEntries.find((e) => e.id === 'een_0143456789abcdef0123456789abcdef')
      expect(deleted).toBeUndefined();
      deleted = updatedElimEntries.find((e) => e.id === 'een_0153456789abcdef0123456789abcdef')
      expect(deleted).toBeUndefined();      
    })
    it('should return updated elimEntries when elimEntries updated', () => {       
      const editedElimEntries = cloneDeep(mockOrigData.elimEntries)      
      const mockAllEnts = cloneDeep(mockAllEntrisOneSquad)
      editedElimEntries[0].fee = '4';      
      editedElimEntries[1].fee = '4';
      ...
      expect(updatedElimEntries).not.toBe(null)
      if (updatedElimEntries === null) return;
      expect(updatedElimEntries).toHaveLength(4);
      expect(updatedElimEntries[0].fee).toBe('4');
      expect(updatedElimEntries[1].fee).toBe('4');
    })    
    it('should return updated elimEntries when elimEntries inserted', () => {       
      const editedElimEntries = cloneDeep(mockOrigData.elimEntries)
      const mockAllEnts = cloneDeep(mockAllEntrisOneSquad)
      const addedBrktEntry1 = {
        ...initElimEntry,        
      ...
      expect(updatedElimEntries).not.toBe(null)
      if (updatedElimEntries === null) return;
      expect(updatedElimEntries).toHaveLength(6);      
      expect(updatedElimEntries[updatedElimEntries.length - 2].fee).toBe('6');
      expect(updatedElimEntries[updatedElimEntries.length - 1].fee).toBe('6');
    })    
    it('should return updated elimEntries when elimEntries deleted,updated, and inserted', () => { 
      const editedElimEntries = cloneDeep(mockOrigData.elimEntries).slice(0, 2)
      const mockAllEnts = cloneDeep(mockAllEntrisOneSquad)
      editedElimEntries[0].fee = '4';      
      editedElimEntries[1].fee = '4';
  ...
  describe('saveEntriesData', () => { 
    ...
    it('should save player entries with inserted player', async () => { 
  ...
  describe('updateAllEntries', () => { 
    ...
    it('should save, insert and delete entries to players, divEntries, potEntries, brktEntries and elimEntries', async () => { 
      ...
      newRow[brkt1NumColName] = 5
      newRow[brkt1FeeColName] = '25'
      newRow[brkt1TimeStampColName] = new Date().getTime()
      newRow[brkt2NumColName] = 5
      newRow[brkt2FeeColName] = '25'
      newRow[brkt2TimeStampColName] = new Date().getTime()
      newRow[elim1FeeColName] = '6'
      newRow[elim2FeeColName] = '6'
      rows.push(newRow)  // insert player, div entry, pot entry, 2 brkt entry, 2 elim entry
      ...
      rows[1][brkt1NumColName] = 5;    // adds a new bracket entry (no num yet for row in grid)
      rows[1][brkt1FeeColName] = '25';       
      rows[1][brkt1TimeStampColName] = new Date().getTime();
      rows[1][elim1FeeColName] = '6';  // adds a new elim entry (no fee yet for row in grid)
      ...
      const allUpdates = updateAllEntries(updatedCounts, testAllData);
      expect(allUpdates).not.toBeNull();
      if (!allUpdates) return;

      expect(allUpdates.players.length).toBe(6);
      // updated
      expect(allUpdates.players[0].first_name).toBe('Anna');
      // not edited
      expect(allUpdates.players[1].first_name).toBe('Betty')  
      expect(allUpdates.players[2].first_name).toBe('Carol')
      expect(allUpdates.players[3].first_name).toBe('Debbie')
      expect(allUpdates.players[4].first_name).toBe('Ella')
      // deleted
      const foundPlayer = allUpdates.players.find(p => p.id === 'ply_0173456789abcdef0123456789abcdef');
      expect(foundPlayer).toBeUndefined();
      // inerted
      expect(allUpdates.players[5].first_name).toBe('Gina');

      expect(allUpdates.divEntries.length).toBe(5);
      // updated
      expect(allUpdates.divEntries[0].fee).toBe('83');
      // not edited
      expect(allUpdates.divEntries[1].fee).toBe('85');
      expect(allUpdates.divEntries[2].fee).toBe('85');
      // deleted
      let foundDivEntry = allUpdates.divEntries.find(d => d.id === 'den_0163456789abcdef0123456789abcdef');
      expect(foundDivEntry).toBeUndefined();
      foundDivEntry = allUpdates.divEntries.find(d => d.id === 'den_0133456789abcdef0123456789abcdef');
      expect(foundDivEntry).toBeUndefined();      
      // inserted
      expect(allUpdates.divEntries[3].fee).toBe('86');
      expect(allUpdates.divEntries[4].fee).toBe('86');

      expect(allUpdates.potEntries.length).toBe(4);
      // updated
      expect(allUpdates.potEntries[0].fee).toBe('15');
      // not edited
      expect(allUpdates.potEntries[1].fee).toBe('20');
      // deleted
      const deltedPotEntry = allUpdates.potEntries.find(p => p.id === 'pen_0133456789abcdef0123456789abcdef');
      expect(deltedPotEntry).toBeUndefined();
      // inserted
      expect(allUpdates.potEntries[2].fee).toBe('21');
      expect(allUpdates.potEntries[3].fee).toBe('21');

      expect(allUpdates.brktEntries.length).toBe(6);
      // updated
      expect(allUpdates.brktEntries[0].num_brackets).toBe(8);
      expect(allUpdates.brktEntries[0].fee).toBe('40');
      // not edited
      expect(allUpdates.brktEntries[1].num_brackets).toBe(4);
      expect(allUpdates.brktEntries[1].fee).toBe('20');
      expect(allUpdates.brktEntries[2].num_brackets).toBe(6);
      expect(allUpdates.brktEntries[2].fee).toBe('30');
      // deleted
      let foundBrktEntry = allUpdates.brktEntries.find(b => b.id === 'ben_0143456789abcdef0123456789abcdef');
      expect(foundBrktEntry).toBeUndefined();
      // inserted
      expect(allUpdates.brktEntries[3].num_brackets).toBe(5);      
      expect(allUpdates.brktEntries[3].fee).toBe('25');
      expect(allUpdates.brktEntries[4].num_brackets).toBe(5);      
      expect(allUpdates.brktEntries[4].fee).toBe('25');
      expect(allUpdates.brktEntries[5].num_brackets).toBe(5);
      expect(allUpdates.brktEntries[5].fee).toBe('25');

      expect(allUpdates.elimEntries.length).toBe(5);
      // updated
      expect(allUpdates.elimEntries[0].fee).toBe('15');
      // not edited
      expect(allUpdates.elimEntries[1].fee).toBe('5'); 
      // deleted
      let deletedElimEntry = allUpdates.elimEntries.find(e => e.id === 'een_0143456789abcdef0123456789abcdef');
      expect(deletedElimEntry).toBeUndefined();
      deletedElimEntry = allUpdates.elimEntries.find(e => e.id === 'een_0153456789abcdef0123456789abcdef');
      expect(deletedElimEntry).toBeUndefined();
      // inserted
      expect(allUpdates.elimEntries[2].fee).toBe('6');
      expect(allUpdates.elimEntries[3].fee).toBe('6');
      expect(allUpdates.elimEntries[4].fee).toBe('6');
    })
