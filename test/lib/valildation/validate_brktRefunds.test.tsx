...
describe("test for brktRefunds validation", () => { 
  ...
  describe("gotBrktRefundData function", () => {
    ...
    it("should return ErrorCode.MISSING_DATA for missing brkt_entry_id", () => {
    ...
    it("should return ErrorCode.MISSING_DATA for missing num_refunds", () => {
      const testBrktRefund = cloneDeep(validBrktRefund);
      testBrktRefund.num_refunds = undefined as any;
      const result = gotBrktRefundData(testBrktRefund);
      expect(result).toEqual(ErrorCode.MISSING_DATA);
    });
    it("should return ErrorCode.MISSING_DATA when num_refunds is not a number", () => {
      const testBrktRefund = cloneDeep(validBrktRefund);
  ...
  describe("validBrktRefundData function", () => {
    ...
    it("should return InvalidData for null brktSeed", () => {
      expect(validBrktRefundData(null as any)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for invalid one_brkt_id", () => {
      const testBrktRefund = cloneDeep(validBrktRefund);
      testBrktRefund.brkt_entry_id = "test";
      expect(validBrktRefundData(testBrktRefund)).toEqual(ErrorCode.INVALID_DATA);
    });
    ...
    it("should return ErrorCode.INVALID_DATA for missing num_refunds", () => {
      const testBrktRefund = cloneDeep(validBrktRefund);
      testBrktRefund.num_refunds = undefined as any;
      expect(validBrktRefundData(testBrktRefund)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for num_refunds not a number", () => {
      const testBrktRefund = cloneDeep(validBrktRefund);
      testBrktRefund.num_refunds = "test" as any;
      expect(validBrktRefundData(testBrktRefund)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for num_refunds not an integer", () => {
      const testBrktRefund = cloneDeep(validBrktRefund);
      testBrktRefund.num_refunds = 1.5;
      expect(validBrktRefundData(testBrktRefund)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for num_refunds too low", () => {
      const testBrktRefund = cloneDeep(validBrktRefund);
      testBrktRefund.num_refunds = -1;
      expect(validBrktRefundData(testBrktRefund)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for num_refunds too high", () => {
      const testBrktRefund = cloneDeep(validBrktRefund);
  ...
  describe('validateBrktRefund function', () => {
    ...
    describe('validateBrktRefund function - missing data', () => { 
      it('should return Missingdata for null brktRefund', () => { 
        expect(validateBrktRefund(null as any)).toEqual(ErrorCode.MISSING_DATA);
      })
      it('should return Missingdata for blank brkt_entry_id', () => { 
        const testBrktRefund = cloneDeep(validBrktRefund);
        testBrktRefund.brkt_entry_id = '';
        expect(validateBrktRefund(testBrktRefund)).toEqual(ErrorCode.MISSING_DATA);
      })
    ...
    describe('validateBrktRefund function - missing data', () => { 
      it('should return InvalidData for invalid brkt_entry_id', () => {        
        const testBrktRefund = cloneDeep(validBrktRefund);
        testBrktRefund.brkt_entry_id = "test";
        expect(validateBrktRefund(testBrktRefund)).toEqual(ErrorCode.INVALID_DATA);
      })
