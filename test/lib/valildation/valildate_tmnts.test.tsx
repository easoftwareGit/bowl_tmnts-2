import {
  ...
  validTmntDates,
  validTmntFkId,
} from "@/lib/validation/tmnts/valildate";
...
describe("tmnt table data validation", () => {
  ...
  describe("validTmntName function", () => {
    ...
    it("should return false for tmnt_name that is too long", () => {
      expect(
        validTmntName("12345678901234567890123456789012345678901234567890"),
      ).toBe(false);
    });
  ...
  describe("validTmntDates function", () => {
    it("should return true for valid start_date and end_date, both the same date", () => {
      ...
      expect(
        validTmntDates(new Date(startDate1Str) as any, startDate1Str as any),
      ).toBe(false);
      expect(
        validTmntDates(startDate1Str, new Date(startDate1Str) as any),
      ).toBe(false);
    });
  ...
  describe("sanitizeTmnt function", () => {
    ...
    it("should return a sanitized tmnt object when tmnt has an invalid id", () => {
      const testTmnt: tmntType = {
        ...mockTmnt,
        id: "test",
      };
      const sanitized = sanitizeTmnt(testTmnt);
      expect(sanitized.id).toEqual("test"); // sanitized, not validated
    });
    ...
    it("should return a sanitized bowl_id for an invalid bowl_id", () => {
      const testTmnt: tmntType = {
        ...mockTmnt,
        bowl_id: "<script>alert('hello')</script>",
      };
      const sanitized = sanitizeTmnt(testTmnt);
      expect(sanitized.bowl_id).toEqual("scriptalerthelloscript"); // sanitized, not validated
    });
    it("should return a sanitized bowl_id for an valid id, but wrong id type", () => {
      const testTmnt: tmntType = {
        ...mockTmnt,
        bowl_id: mockTmnt.user_id,
      };
      const sanitized = sanitizeTmnt(testTmnt);
      expect(sanitized.bowl_id).toEqual(mockTmnt.user_id);
    });
    it("should return a blank user_id for an invalid user_id", () => {
      const testTmnt: tmntType = {
        ...mockTmnt,
        user_id: "<script>alert('hello')</script>",
      };
      const sanitized = sanitizeTmnt(testTmnt);
      expect(sanitized.user_id).toEqual("scriptalerthelloscript");
    });
    ...
    it("should return a blank user_id for an valid id, but wrong id type", () => {
      ...
      const sanitized = sanitizeTmnt(testTmnt);
      expect(sanitized.user_id).toEqual(mockTmnt.bowl_id); // sanitized, not validated
    });
  });
