...
describe("test for brktSeeds validation", () => {
  ...
  describe("gotBrktSeedData function", () => {
    ...
    it("should return ErrorCode.MISSING_DATA for missing one_brkt_id", () => {
      ...
      expect(result).toEqual(ErrorCode.MISSING_DATA);
    });
    ...
    it("should return ErrorCode.MISSING_DATA for missing seed", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
      testBrktSeed.seed = undefined as any;
      const result = gotBrktSeedData(testBrktSeed);
      expect(result).toEqual(ErrorCode.MISSING_DATA);
    });
    it("should return ErrorCode.MISSING_DATA when seed is not a number", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
  ...
  describe("validBrktSeedData function", () => {
    ...
    it("should return ErrorCode.INVALID_DATA for invalid one_brkt_id", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
      testBrktSeed.one_brkt_id = "test";
      expect(validBrktSeedData(testBrktSeed)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for vaild one_brkt_id, but not a oneBrkt id", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
      testBrktSeed.one_brkt_id = userId;
      expect(validBrktSeedData(testBrktSeed)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for one_brkt_id is blank", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
      testBrktSeed.one_brkt_id = "";
      expect(validBrktSeedData(testBrktSeed)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for missing seed", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
      testBrktSeed.seed = undefined as any;
      expect(validBrktSeedData(testBrktSeed)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for seed not a number", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
      testBrktSeed.seed = "test" as any;
      expect(validBrktSeedData(testBrktSeed)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for seed not an integer", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
      testBrktSeed.seed = 1.5;
      expect(validBrktSeedData(testBrktSeed)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for seed too low", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
      testBrktSeed.seed = -1;
      expect(validBrktSeedData(testBrktSeed)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for seed too high", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
      testBrktSeed.seed = defaultBrktPlayers;
      expect(validBrktSeedData(testBrktSeed)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for invalid player_id", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
      testBrktSeed.player_id = "test";
      expect(validBrktSeedData(testBrktSeed)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for vaild player_id, but not a player id", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
      testBrktSeed.player_id = userId;
      expect(validBrktSeedData(testBrktSeed)).toEqual(ErrorCode.INVALID_DATA);
    });
    it("should return ErrorCode.INVALID_DATA for player_id is blank", () => {
      const testBrktSeed = cloneDeep(validBrktSeed);
  ...
  describe('validateBrktSeed function', () => {
    ...
    describe('validateBrktSeed function - missing data', () => { 
      it('should return Missingdata for null brktSeed', () => { 
        expect(validateBrktSeed(null as any)).toEqual(ErrorCode.MISSING_DATA);
      })
      it('should return Missingdata for blank one_brkt_id', () => { 
        const testBrktSeed = cloneDeep(validBrktSeed);
        testBrktSeed.one_brkt_id = '';
        expect(validateBrktSeed(testBrktSeed)).toEqual(ErrorCode.MISSING_DATA);
      })
    ...
    describe('validateBrktSeed function - missing data', () => { 
      it('should return InvalidData for invalid one_brkt_id', () => {        
        const testBrktSeed = cloneDeep(validBrktSeed);
        testBrktSeed.one_brkt_id = "test";
        expect(validateBrktSeed(testBrktSeed)).toEqual(ErrorCode.INVALID_DATA);
      })
