...
describe("tests for brktEntry validation", () => {
  describe("gotBrktEntryData()", () => {
    ...
    it("should return ErrorCode.NONE when fee is missing and not required", () => {
      const zeroTimeStamp = cloneDeep(validBrktEntry);
      zeroTimeStamp.fee = '';
      const errorCode = gotBrktEntryData(zeroTimeStamp, false);
      expect(errorCode).toBe(ErrorCode.NONE);
    });
    ...
    it("should return ErrorCode.MISSING_DATA when fee is missing and required", () => {
      const testBrktEntry = {
        ...validBrktEntry,
        fee: null as any,
      };
  ...
  describe("validBrktEntryFee()", () => {
    ...
    it("should return false when fee is zero", () => {
      expect(validBrktEntryFee("0")).toBe(false);
    });
  ...
  describe("validBrktEntryData()", () => {
    ...
    it("should return ErrorCode.NONE when all data is valid, fee is missing and NOT required", () => {
      const noFee = cloneDeep(validBrktEntry);
      noFee.fee = null as any;
      const errorCode = validBrktEntryData(noFee, false);
      expect(errorCode).toBe(ErrorCode.NONE);
    });
  ...
  describe("validateBrktEntry()", () => {
    describe("valid data", () => {
      ...
      it("should return ErrorCode.NONE when fee is missing and NOT required", () => {
        ...
        const errorCode = validateBrktEntry(testBrktEntry, false);
        expect(errorCode).toBe(ErrorCode.NONE);
      });
    });    
  ...
  describe("validateBrktEntries()", () => {
    ...
    it("should return ErrorCode.INVALID_DATA when num_refunds is not a number (sanitized to 0)", () => {
      const brktEntriesToValidate = cloneDeep(mockBrktEntriesToPost);
    ...
    it("should return ErrorCode.INVALID_DATA when time_stamp is invalid", () => {
      const brktEntriesToValidate = cloneDeep(mockBrktEntriesToPost);
      brktEntriesToValidate[1].time_stamp = "abc" as any;
      const validBrktEntries: validBrktEntriesType = validateBrktEntries(
        brktEntriesToValidate,
    ...
    it("should return ErrorCode.INVALID_DATA when time_stamp is null", () => {
      const brktEntriesToValidate = cloneDeep(mockBrktEntriesToPost);
